...
class Dict(object):
    def __init__(self, opt):
        self.opt = opt
        self.minfreq = opt.get("dict_min_freq", 0)
    ...
    def build(self, sort=True):
        ...
        if os.path.exists(save_path):
            ...
            if self.null_tok:
                ...
                self.freq[self.null_tok] = 1000002
            if self.start_tok:
                self.add_token(self.start_tok)
                self.freq[self.start_tok] = 1000001
            if self.end_tok:
                self.add_token(self.end_tok)
                self.freq[self.end_tok] = 1000000
